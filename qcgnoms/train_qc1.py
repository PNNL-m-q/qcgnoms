...
def df_to_data(row,qc_data):
    # openbabel atomic masses
    am_idx=np.array([1.00794, 10.811, 12.0107, 14.0067, 15.9994, 18.9984032, 22.98977, 28.0855, 30.973761, 32.065, 35.453, 55.845, 74.9216, 78.96, 79.904, 107.8682, 118.701, 126.90447, 195.078])
    x=qc_data.x.values
    ...
    if len(x) == 0:
        return None
    #only use the first value x[0] for the atomic mass
    #rebuild x with the one hot encode +1 for the collision energy
    ...
    x = oh_x
    #delete the integer bond orders
    edge_attr = np.delete(edge_attr,[0],1)
